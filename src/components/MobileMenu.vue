<template>
    <div>
        <div v-if="isOpen" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" @click="$emit('close')"></div>

        <Transition appear name="slide-x">
            <div v-if="isOpen" class="fixed top-0 left-0 h-full w-72 bg-gray-900/95 backdrop-blur-lg z-50 shadow-2xl transform-gpu will-change-transform">
                ...
                <nav class="p-4 overflow-y-auto" style="max-height: calc(100vh - 80px);">
                    <ul class="flex flex-col gap-2">
                        <li
                            ...
                            :key="index"
                            class="text-white cursor-pointer hover:bg-gray-800 hover:translate-x-2 transition-all duration-300 font-lg border border-gray-800 px-4 py-3 rounded-lg active:scale-95"
